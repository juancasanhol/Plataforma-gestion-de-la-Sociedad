    ...
    $.ajax({
        ...
        success: function (respuesta) {
            ...
            $.each(respuesta, function (i, option) {
                if (option.idTipodocumento !== undefined) {
                    $("#DesplegablesTipoDoc").append('<option value="' + option.idTipodocumento + '">' + option.tipodocumento + '</option>');
                }
                if (option.idSexo !== undefined) {
                    $("#DesplegablesSexo").append('<option value="' + option.idSexo + '">' + option.sexo + '</option>');
                }
                if (option.idPaisorigen !== undefined) {
                    $("#DesplegablesPaisOrigen").append('<option value="' + option.idPaisorigen + '">' + option.paisorigen + '</option>');
                }
                if (option.nacionalidad !== undefined) {
                    $("#DesplegablesNacionalidad").append('<option value="' + option.idNacionalidad + '">' + option.nacionalidad + '</option>');
                }
                if (option.minoriaetnica !== undefined) {
                    $("#DesplegablesMinoria").append('<option value="' + option.idMinoriaetnica + '">'+ option.minoriaetnica + '</option>');
                }
                if (option.usuarios !== undefined) {
                    $("#DesplegablesPersonaReferencia").append('<option value="' + option.idUsuarios + '">'+ option.usuarios + '</option>');
                }
                if (option.origenIngresos !== undefined) {
                    $("#DesplegablesOrigenIngresos").append('<option value="' + option.idOrigenIngresos + '">' + option.origenIngresos + '</option');
                }
                if (option.tipoCarnetConducir !== undefined) {
                    $("#tiposCarnet").append('<tr><td>' + option.tipoCarnetConducir + '</td><td><button type="button" id=' + option.tipoCarnetConducir + ' class="btn btn-success addCarnet">añadir</button></td></tr>');
                }
                if (option.nombreBolsa !== undefined) {
                    $("#nombreBolsa").append('<tr><td>' + option.nombreBolsa + '</td><td><button type="button" id=' + option.nombreBolsa + ' class="btn btn-success addBolsa">añadir</button></td></tr>');
                }
